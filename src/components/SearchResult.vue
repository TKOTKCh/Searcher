...
<!--                      <el-link class="title_caption" :underline='false' v-html="lightFn(item.POLICY_TITLE,search_word_not_contain_filter)" style="font-weight: bolder;font-size: 20px" target="_blank" @click="intoContent(item)"></el-link>-->
                      <el-link class="title_caption" :underline='false' v-html="lightFn(item.POLICY_TITLE,red)" style="font-weight: bolder;font-size: 20px" target="_blank" @click="intoContent(item)"></el-link>
  ...
  methods: {
    ...
    lightFn(originStr, target) {
      for(var i=0;i<target.length;i++){
        if(originStr.indexOf(target[i].word)>-1){
          console.log("匹配到了"+target[i].word)
          originStr=originStr.replaceAll(
              target[i].word,
              '<span style="color:red;">' + target[i].word + "</span>"
          );
        }
      }
      console.log(originStr)
      return originStr
      console.log("没匹配到，使用原来的字符串替换:" + this.search_word_not_contain_filter)

      return originStr.replace(
          this.search_word_not_contain_filter,
          '<span style="color:red;">' + this.search_word_not_contain_filter + "</span>"
      );
    },
    //以图搜图的展示
    //上传图片后的回调函数
    ...
    getRed(){
      axios.get("http://localhost:8081/bm25/completion?query=" + this.search_word)
          .then((response) => {
    ...
    async search() {
      if(this.search_word==''){
        ...
        if (this.info.data&&this.info.data&&this.info.data.count!=0) {
          ...
          this.red=this.info.data.segments;
        } else {
          this.imgAndCaption = [];
          this.recordsNum = 0;
        }
